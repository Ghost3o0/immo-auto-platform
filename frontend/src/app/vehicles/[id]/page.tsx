...
export default function VehicleDetailPage() {
  ...
  const checkFavorite = async () => {
    try {
      const response = await favoritesApi.check({ vehicleId: vehicle.id });
  ...
  const handleFavorite = async () => {
    ...
    try {
      if (isFavorite && favoriteId) {
        ...
        const response = await favoritesApi.add({ vehicleId: vehicle.id });
        setIsFavorite(true);
      ...
      <div className="grid gap-8 md:grid-cols-3">
        ...
        <div className="space-y-6 md:col-span-2">
          <div>
            ...
            <h1 className="mb-2 text-3xl font-bold">{vehicle.title}</h1>
          ...
          <div className="text-3xl font-bold text-primary">
            ...
            {isRent && <span className="text-lg font-normal">/jour</span>}
          </div>
                ...
                {vehicle.power && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{vehicle.power} CV</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              {vehicle.user && (
                <div className="mb-4 flex items-center gap-4">
                  <Avatar className="h-12 w-12">
                    {vehicle.user.avatar ? (
                      <AvatarImage src={vehicle.user.avatar} alt={vehicle.user.name} />
                    ) : null}
                    <AvatarFallback>
                      {vehicle.user.name
                        .split(' ')
                  ...
                  <div>
                    <p className="font-semibold">{vehicle.user.name}</p>
                    <Link
                      href={`/users/${vehicle.user.id}`}
              ...
              <div className="space-y-2">
                ...
                {vehicle.user?.email && (
                  <Button variant="outline" className="w-full" asChild>
                    <a href={`mailto:${vehicle.user.email}`}>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                Publiée le {formatDate(vehicle.createdAt)}
              </div>
            </CardContent>
          </Card>
        </div>
